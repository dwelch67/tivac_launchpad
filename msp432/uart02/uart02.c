...
#define UART2_BASE 0x40001800
#define UCA2CTLW0   (UART2_BASE+0x00)
#define UCA2BRW     (UART2_BASE+0x06)
#define UCA2MCTLW   (UART2_BASE+0x08)
#define UCA2RXBUF   (UART2_BASE+0x0C)
#define UCA2TXBUF   (UART2_BASE+0x0E)
#define UCA2IE      (UART2_BASE+0x1A)
#define UCA2IFG     (UART2_BASE+0x1C)
...
{
    PUT8(P3SEL1_L,GET8(P3SEL1_L)&0xF3);
    PUT8(P3SEL0_L,GET8(P3SEL0_L)|0x0C);

    PUT16(UCA2CTLW0,0x0081);
    //PUT16(UCA2BRW,104); //12000000/115200 = 104
    PUT16(UCA2BRW,26); //3000000/115200 = 26
    PUT16(UCA2MCTLW,0x0000);
    PUT16(UCA2IE,0);
    PUT16(UCA2CTLW0,0x0081);
    PUT16(UCA2CTLW0,0x0080);
}
//------------------------------------------------------------------------
static void uart2_send ( unsigned int x )
{
    while(1)
    {
        if(GET16(UCA2IFG)&2) break;
    }
    PUT16(UCA2TXBUF,x);
}
//------------------------------------------------------------------------
static unsigned int uart2_recv ( void )
{
    while(1)
    {
        if(GET16(UCA2IFG)&1) break;
    }
    return(GET16(UCA2RXBUF));
}
